...
<body>
				...
				<div class="modal-body">
					...
					<center>
						<table>
							...
							<tr>
								<td><input type="hidden" name="category" value="samsung"></td>
							</tr>
						</table>
					</center>
				</div>
	...
	<div id="rightcontent" style="position:absolute; top:10%;">
			<div class="alert alert-info"><center><h2>SamSung</h2></center></div>
			<br />
			...
			<table class="table table-hover">
				...
				<?php
					...
					$query = $conn->query("SELECT * FROM `product` WHERE category='samsung' ORDER BY product_id DESC") or die(mysqli_error($db));
				...
				<tr class="del<?php echo $id?>">
					...
					<?php
					$query1 = $conn->query("SELECT * FROM `stock` WHERE product_id='$id'") or die(mysqli_error($db));
 ...
 $result = $conn->query("SELECT * FROM `stock` WHERE product_id='$pid'") or die(mysqli_error($db));
 $row = $result->fetch_array();
  ...
  $que = $conn->query("UPDATE `stock` SET `qty` = '$total' WHERE `product_id`='$pid'") or die(mysqli_error($db));
